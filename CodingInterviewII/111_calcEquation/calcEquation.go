...
func calcEquation(equations [][]string, values []float64, queries [][]string) []float64 {
	...
	// 执行 Floyd 算法
	for k := range calculationGraph {
		for i := range calculationGraph {
			for j := range calculationGraph {
				if calculationGraph[i][k] > 0 && calculationGraph[k][j] > 0 {
					calculationGraph[i][j] = calculationGraph[i][k] * calculationGraph[k][j]
				}
			}
		}
	}
	...
	for i := 0; i < queriesNum; i++ {
		sourceIndex, sourceExist := variable2Index[queries[i][0]]
		targetIndex, targetExist := variable2Index[queries[i][1]]
		if !sourceExist || !targetExist || calculationGraph[sourceIndex][targetIndex] == 0 {
			result[i] = -1.0
		} else {
			result[i] = calculationGraph[sourceIndex][targetIndex]
		}
	}
	return result
}
